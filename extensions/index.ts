...
import { installContextManagement } from "./context-management.ts";
...
function instruction(customInstructions: string | undefined): string {
  ...
  return custom ? `${COMPACTION_INSTRUCTION}\n\nAdditional compaction focus:\n${custom}` : COMPACTION_INSTRUCTION;
}

export default function sharedContextManagement(pi: ExtensionAPI): void {
  installContextManagement(pi);
  pi.on("session_before_compact", async (event, ctx) => {
    try {
      const projectionToken = randomUUID();
      pi.events.emit(REQUEST_PROJECTION_EVENT, { action: "authorize", token: projectionToken });
      const response = await (async () => {
        try {
          return await ctx.completeFromLatestSettledRequest(instruction(event.customInstructions), { signal: event.signal });
        } finally {
          pi.events.emit(REQUEST_PROJECTION_EVENT, { action: "release", token: projectionToken });
        }
      })();
      const summary = contentText(response.content).trim();
      if (!summary) throw new Error("The continuation returned no summary text");
      const modifiedFiles = new Set([...event.preparation.fileOps.written, ...event.preparation.fileOps.edited]);
      return {
        compaction: {
          summary,
          ...
          details: {
            ...
            readFiles: [...event.preparation.fileOps.read].filter((path) => !modifiedFiles.has(path)).sort(),
      ...
      console.warn(`[pi-context-management] warm-prefix compaction failed: ${String(error)}`);
      throw error;
    }
  });
}
